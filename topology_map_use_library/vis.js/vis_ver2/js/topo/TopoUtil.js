var TopoUtil = function(){
	...
	this.defaultOptions = {
		...
		groups:{
			...
			'Distribution Access Switches' :{background:"#ffffff", border:"#008bc7"},
...
TopoUtil.prototype.setNodeEdgeData = function(){
	...
	for(k in handler.networkData){
		if(!k.includes("group")){
			...
			var size = k=='root' ? 35 : 25;
			var node = {
				id : k,
				label : data.name,
				image : handler.DIR + data.img,
				shape : shape,
				title : title,
				group:group,
				x:data.x,
			...
			var data = handler.networkData[k];
			var shape = 'square';
			var len = handler.EDGE_LENGTH_MAIN;
			var group = data.group != '' ? data.group : '';
			var toID = [];
			...
			var node = handler.setCustomNodes({id : k, label:data.name, group:group, x:data.x, y:data.y});
			handler.nodes.push(node);
	...
	/**
		CUSTOM SHAPE
		@vis-network.js ctxRenderer extends
	**/
TopoUtil.prototype.setCustomNodes = function({id, group, label, x, y, values}){
	var handler = this;

	var id = id || Topo.guid();
	var group = group == undefined ? '' : group;
	var label = label == undefined ? '' : label;

	var {width, height, x, y} = handler.getGroupBoxSize(label);

	if(width == undefined)console.log(label)
	var node = {
		id:id,
		shape:'custom',
		group:group,
		label:label,
		x:x, y:y,
		ctxRenderer:({ctx, x, y, state:{selected, hover}, style, label, id, group, })=>{
			ctx.save();
			ctx.restore();
			const r = style.borderRadius || 5;
...
TopoUtil.prototype.getGroupBoxSize = function(groupID){
	if(groupID === undefined) return;

	var handler = this;
	var xPos = [], yPos = [], isSwitch = false, isExist = false;
	var width, height, x, y;

	for(key in handler.networkData){
		var data = handler.networkData[key];
		if(data.group === groupID){
			xPos.push(data.x); yPos.push(data.y);
	...
	if(isExist === true){
		var maxX = Math.max(...xPos); var minX = Math.min(...xPos);
		var maxY = Math.max(...yPos); var minY = Math.min(...yPos);
...
TopoUtil.prototype.eventListener = function(){
	...
	handler.network.on("dragging", (ev)=>{
		if(ev.nodes.length == 1 && ev.nodes[0].includes("group")){
			var nodes = [];
			var group = handler.networkData[ev.nodes[0]].name;
			for(k in handler.networkData){
				if(handler.networkData[k].group != undefined && handler.networkData[k].group === group){
					//같이 움직일꺼
					if(!ev.nodes.includes(k))
						ev.nodes.push(k);
				}
			}
		}
